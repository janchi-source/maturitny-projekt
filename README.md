...
**Stručný popis:** Interný systém na správu projektov s úlohami, dokumentmi a rolami. Umožňuje nahrávať PDF a DOCX súbory a pracovať s nimi cez AI chat, sumarizáciu a vyhľadávanie. Všetko beží lokálne, dáta neopúšťajú firmu.

**Cieľ projektu:** Prepojiť projektový manažment s prácou nad dokumentmi tak, aby tím v jednom nástroji plánoval, ukladal a cez chat rýchlo našiel kľúčové informácie z príloh. Má to skrátiť čas hľadania informácií a dať rozhodnutiam jasné odkazy na zdroje.
...
| Databáza | SQLite (cez SQLAlchemy ORM) |
| Ukladanie súborov | Lokálny súborový systém (`uploads/`) |
| AI / LLM | Ollama (lokálne). **Implementuje vlastník projektu** |
...
- **AI chat nad dokumentmi:** Otázky a odpovede s citáciami pasáží, sumarizácia dokumentov, vyhľadávanie termínov *(AI logiku implementuje vlastník)*
- **Role a oprávnenia:** admin, owner, advocate, koncipient, sekretariát
- **Dashboard:** Štatistiky v reálnom čase, priebeh projektov, posledné úlohy, feed AI insightov, posledné dokumenty
...
│   │   └── ai_chat.py           # Modely AI chat session a message
│   ├── blueprints/
│   │   ├── __init__.py
...
│   │   ├── ai_chat.py           # Routy pre AI chat (stub endpointy pre vlastníka)
│   │   └── team.py              # Routy pre správu tímu / používateľov
│   ├── services/
│   │   ├── __init__.py
│   │   ├── document_service.py  # Logika extrakcie textu z PDF/DOCX
│   │   └── ai_service.py        # Stub AI služba (dopĺňa vlastník)
│   ├── templates/
...
│   │   │   ├── ai_insight.html  # Komponenta pre položku AI insight feedu
...
│           └── chat.js          # Interakcie AI chat UI (fetch API)
└── uploads/                     # Nahraté dokumenty (v gitignore)
```

---

## Dizajnové pravidlá (podľa `template.html`)
...
## Body AI integrácie (pre vlastníka)

Nasledujúce súbory obsahujú stuby/placeholdery, ktoré vlastník doplní o Ollama-based AI logiku:

1. **`app/services/ai_service.py`** – jadro AI logiky:
   - `chat()` – RAG-based Q&A nad textom dokumentu s extrakciou citácií
   - `summarize()` – sumarizácia dokumentu
   - `search_terms()` – sémantické vyhľadávanie termínov naprieč dokumentmi
2. **`app/blueprints/ai_chat.py`** – endpointy už napojené na volanie metód `AIService`
3. **`app/templates/ai_chat/index.html`** – chat UI je už postavené, posiela a prijíma JSON
4. **`app/static/js/chat.js`** – frontendové fetch volania sú už implementované

**Očakávaný AI flow:**
1. Používateľ nahrá dokument → text sa extrahuje a uloží do DB
2. Používateľ otvorí AI chat → vyberie kontext dokumentu (alebo všetky dokumenty projektu)
3. Používateľ pošle správu → backend odošle správu + text dokumentu do LLM
4. LLM vráti odpoveď s citáciami → zobrazí sa v chate
