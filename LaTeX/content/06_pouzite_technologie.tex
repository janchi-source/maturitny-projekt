...
\chapter{Použité technológie}
        ...
        Aplikácia je postavená na modernom zásobniku technológií, ktorý pokrýva backendové
spracovanie požiadaviek, frontendové zobrazenie a~správu databázy. Výber technológií bol
podmienený požiadavkou na lokálnu prevádzku, jednoduchosť nasadenia a~dostupnosť
kvalitných knižníc pre AI~integráciu.

\section{Backend}

        Backendová vrstva aplikácie je implementovaná v~jazyku Python s~využitím
...
jednoduchosťou a~čitateľnosťou kódu. Bol vytvorený v~roku 1991 Guidom van Rossum a~v~súčasnosti
...
webových aplikácií. Na rozdiel od Django, ktorý je \emph{all-in-one} riešenie, ponúka Flask
minimalistický prístup, ktorý umožňuje vývojárom zvoliť si vlastné knižnice a~komponenty.

        Flask je postavený na Werkzeug (WSGI toolkit) a~Jinja2 (template engine) a~poskytuje
...
na HTTPS s~SSL/TLS šaním pre zabezpečené komunikácie.
...
\begin{itemize}
  \item \textbf{SQLAlchemy} -- ORM (Object-Relational Mapping) knižnica pre komunikáciu s~databázou
  \item \textbf{Flask-SQLAlchemy} -- integrácia SQLAlchemy s~Flask
  \item \textbf{Flask-Login} -- spravovanie používateľských relácií a~autentifikácie
  \item \textbf{Werkzeug} -- WSGI toolkit, ktorý Flask používa na spracovanie HTTPS požiadaviek
        s~SSL/TLS podporou
  \item \textbf{Flask-WTF} -- CSRF ochrana vo formulároch
  \item \textbf{Flask-Caching} -- in-memory cache pre zrýchlenie opakovaných požiadaviek
  \item \textbf{ollama} -- integrácia s~lokálnym LLM serverom pre AI~chat funkcionalitu
  \item \textbf{google-genai} -- integrácia s~Google Generative AI API pre náročnejšie úlohy
  \item \textbf{python-docx} -- čítanie a~písanie Word dokumentov (.docx)
  \item \textbf{PyPDF2} -- spracovanie PDF súborov
  \item \textbf{python-dotenv} -- správa citlivých informácií (API kľúče, databázové pripojenia)
  \item \textbf{Jinja2} -- template engine pre dynamické HTML stránky
\end{itemize}
...
\section{Frontend}
        ...
        Frontend aplikácie je postavený na moderných webových technológiách, ktoré
zabezpečujú interaktívne a~responzívne používateľské rozhranie. Hlavným zameraním
...
\end{itemize}
        ...
        Aplikácia používa modulárnu štruktúru šablón s~hlavným \texttt{base.html} súborom,
z~ktorého dedičnosti všetky ďalšie šablóny. Komponenty ako header, sidebar, modálne
dialógy a~karty sú oddelené do samostatných šablón pre lepšiu udržiavateľnosť.

\subsection{JavaScript a Interaktivita}

        JavaScript zabezpečuje dynamické správanie aplikácie bez nutnosti obnovy celej
stránky. Kľúčové JavaScript moduly:
...
\section{Databáza}
        ...
        Aplikácia používa relačnú databázu pre trvalé uloženie všetkých dát. Model databázy
je definovaný pomocou SQLAlchemy ORM modelu, ktorý umožňuje pracovať s~databázovými
tabuľkami ako s~Python objektmi.

\subsection{SQLite}

        Projekt používa SQLite ako primárny databázový systém. SQLite je ľahký, spoľahlivý
...
\begin{itemize}
  \item \textbf{User} -- používatelia s~rolami (admin, manažér, člen tímu)
  \item \textbf{Project} -- projekty s~metadátami a~nastaveniami
  \item \textbf{Task} -- úlohy s~prioritou, statusom a~priradením
  \item \textbf{Document} -- dokumenty s~verzionovaním a~komentármi
  \item \textbf{Comment} -- komentáre na úlohách a~dokumentoch
  \item \textbf{Notification} -- upozornenia pre používateľov
  \item \textbf{Team} -- tímová štruktúra s~oprávneniami
  \item \textbf{Sprint} -- sprint plánovanie pre iteratívny vývoj
\end{itemize}

\section{Ostatné nástroje}

        Okrem hlavných technológií, projekt využíva niekoľko ďalších nástrojov a~služieb
...
\begin{itemize}
  \item \textbf{Ollama} -- lokálny LLM server s~open-source modelmi (napr. Gemma~3)
  \item \textbf{Google Generative AI} -- cloudové AI~API pre náročnejšie úlohy
\end{itemize}

        Tieto integrácie umožňujú AI~chat funkcionalitu a~automatické generovanie informácií.
Predvolený model je \texttt{gemma3:1b} bežiaci lokálne, čo zabezpečuje plnú súkromnosť
...
\end{itemize}
        ...
        Pomocou týchto knižníc môže aplikácia extrahovať text z~dokumentov a~umožniť
AI~modelom pracovať s~ich obsahom.

\subsection{Verzia Kontrola a Deployment}

        Git je používaný pre verzionovanie kódu a~správu vývojového procesu. Každá
funkčná zmena je zaznamenaná v~samostatnom commite s~popisom vykonaných úprav.

\subsection{Bezpečnosť a Overovanie}

\begin{itemize}
  \item \textbf{Flask-Login} -- session management a~autentifikácia
  \item \textbf{Flask-WTF} -- CSRF ochrana vo formulároch
  \item \textbf{python-dotenv} -- správa citlivých informácií (API keys, databázové pripojenia)
\end{itemize}
